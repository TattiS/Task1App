			...
			using (WebClient webClient = new WebClient())
			{
				...
				try
				{
					var entitiesList = from user in users
									   ...
									   select new SomeEntity { Id = user.Id, Name = user.Name, Avatar = user.Avatar, CreatedAt = user.CreatedAt, Email = user.Email, MyAddress = address, Posts = postsList, Todos = tList };

					return entitiesList; //entitiesList.ToList<SomeEntity>();
				}
				catch (Exception)
				{
